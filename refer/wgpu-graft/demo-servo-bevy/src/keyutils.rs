...
// Adapted from Servo's servoshell/desktop/keyutils.rs via
// `demo-servo-winit/src/keyutils.rs` (Mozilla Public License 2.0).
//
// Maps Bevy 0.19 keyboard input to Servo's `keyboard_types`-based
// KeyboardEvent. Bevy carries its own W3C-aligned `Key` / `KeyCode` enums
// (converted from winit by `bevy_winit`), so this mirrors the winit mapping
// arm-for-arm against the Bevy types instead. Both follow the W3C UI Events
// specification, so named keys and physical codes map 1:1; anything the two
// specs disagree on falls into `Unidentified` via the wildcards.
