...
pub struct Window {
    ...
    /// Stack of platform view pointers for surface rendering. When a surface is being
    /// painted, its NSView is pushed here so that `raw_native_view_ptr()` returns
    /// the surface's view instead of the main window's view.
...
impl Window {
    ...
    /// Used by hosted platform subviews to add platform-owned child views.
    ///
    /// When a GpuiSurface is being painted, this returns the surface's GPUISurfaceView
    /// instead of the main window's view, so hosted platform subviews automatically parent
    ...
    pub fn native_toolbar_item_bounds(&self, _item_id: &str) -> Option<Bounds<Pixels>> {
        None
    }
    ...
    fn draw_surfaces(&mut self, cx: &mut App) {
        ...
        for id in surface_ids {
            ...
            // Push a platform-view override so hosted subviews parent to the surface's NSView
