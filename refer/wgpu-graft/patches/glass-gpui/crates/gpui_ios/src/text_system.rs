...
pub(crate) struct IosTextSystem(RwLock<IosTextSystemState>);
...
impl PlatformTextSystem for IosTextSystem {
        ...
        if let Some(font_id) = lock.font_selections.get(font) {
            ...
            let candidates = if let Some(font_ids) = lock.font_ids_by_font_key.get(&font_key) {
                ...
                match lock.load_family(&font.family, &font.features, font.fallbacks.as_ref()) {
                    ...
                    Err(e) => {
                        log::error!(
                            "[IosTextSystem] FAILED to load family {:?}: {}",
                            font.family,
                            e
                        );
                        return Err(e);
            ...
            let ix = font_kit::matching::find_best_match(
                ...
                &font_kit::properties::Properties {
                    style: fontkit_style_from_font_style(font.style),
                    weight: fontkit_weight_from_font_weight(font.weight),
    ...
    fn font_metrics(&self, font_id: FontId) -> FontMetrics {
        font_metrics_from_metrics(self.0.read().fonts[font_id.0].metrics())
    }

    fn typographic_bounds(&self, font_id: FontId, glyph_id: GlyphId) -> Result<Bounds<f32>> {
        Ok(bounds_from_rect_f(
...
impl IosTextSystemState {
    ...
    fn load_family(
        ...
        let name = font_name_with_fallbacks(name, ".AppleSystemUIFont");
    ...
    fn raster_bounds(&self, params: &RenderGlyphParams) -> Result<Bounds<DevicePixels>> {
        ...
        Ok(bounds_from_rect_i_device_pixels(font.raster_bounds(
            params.glyph_id.0,
...
fn font_metrics_from_metrics(metrics: Metrics) -> FontMetrics {
    FontMetrics {
        ...
        bounding_box: bounds_from_rect_f(metrics.bounding_box),
    }
}

fn bounds_from_rect_f(rect: RectF) -> Bounds<f32> {
    Bounds {
...
fn bounds_from_rect_i_device_pixels(rect: RectI) -> Bounds<DevicePixels> {
    Bounds {
...
fn fontkit_weight_from_font_weight(value: FontWeight) -> FontkitWeight {
    FontkitWeight(value.0)
}

fn fontkit_style_from_font_style(style: FontStyle) -> FontkitStyle {
    match style {
