...
// NOTE: `impl From<Error> for fmt::Error` intentionally removed.
// serde_fmt 1.1.0 added this impl, but it creates an ambiguity with the
// blanket `impl<T> From<T> for T` in core when Rust infers types in the
// `ToCss` derive macro (used by stylo). Removing it restores the unique
// blanket-impl resolution that older Rust compilers used.
// The practical effect: serde_fmt::Error can no longer be directly converted
// to fmt::Error via `?`; callers that rely on this are rare and can use
// `.map_err(|_| fmt::Error)` instead.
