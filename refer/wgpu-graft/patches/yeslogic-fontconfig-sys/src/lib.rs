// Patched copy of yeslogic-fontconfig-sys 6.0.0.
//
// Upstream emits EITHER root-level `extern "C" { pub fn FcXxx ... }` declarations
// (when its `dlopen` feature is off) OR a `Fc` struct of runtime-loaded function
// pointers (when `dlopen` is on) — never both. Cargo's feature unification then
// breaks any combination where one consumer wants each. In this repo:
//
//   - `servo-fonts` imports `FcConfigGetCurrent`, etc. as root-level extern fns.
//   - `fontique` (pulled via xilem→parley) enables the `dlopen` feature and
//     dispatches through `statics::LIB`.
//
// Under feature unification, fontique's request forces `dlopen` on globally, the
// extern block disappears, and servo-fonts fails to compile.
//
// This patched crate emits BOTH unconditionally: the root-level extern block
// (for direct callers) AND the `Fc` struct + `statics::LIB` (for runtime
// dispatch). The `dlopen` cargo feature is preserved as a no-op so dependents
// that toggle it still compile.
//
// The body below is the upstream lib.rs with two targeted changes:
//   1. `pub mod statics` is no longer gated on `feature = "dlopen"`.
//   2. The single `dlib::external_library!(...)` invocation is replaced by
//      separate `dlib::link_external_library!(...)` + `dlib::dlopen_external_library!(...)`
//      calls so both expansions land in the same crate.
//
// Copyright 2013 The Servo Project Developers. See the LICENSE file at the
// top-level directory of this distribution. Licensed under the MIT license.
...
// ---------------------------------------------------------------------------
// Function declarations. The list is duplicated across the two macro calls
// because Rust's declarative macros can't be passed as arguments to other
// macros. Keep the two lists in sync if you ever update them.
// ---------------------------------------------------------------------------

dlib::link_external_library!("fontconfig",
    functions:
...
dlib::dlopen_external_library!(Fc,
    functions:
        fn FcBlanksCreate() -> *mut FcBlanks,
